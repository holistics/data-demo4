...
**Recommended order on a call:** C (the recommendation) → A and B only if the room wants to see why the fact-table route was dropped.

### There used to be a Solution D

D was the original walkthrough (Dong Le, Aug 2026) and C's ancestor: the same architecture — `dim_dates.extend()`, all-`@sql` fiscal columns — with a strictly smaller dimension set. It was removed on 2 Sep 2026 because maintaining two copies of one idea bought nothing and its labels had already drifted (`FY26` where C says `FY 2026`). Recover it from git history if a minimal teaching version is ever wanted back; the deletion also removed its cloud page `seed_fiscal_year_demo` from the `tsco` dev branch.

### Duplication is deliberate

A and B each carry their own copy of the eleven inherited-field re-declarations that `extend()` on a fact table forces. That duplication is the price of "one folder, one demo, no cross-wiring" and it is worth paying here. Do **not** re-introduce a shared model to remove it — the whole point of the layout is that a customer can be shown one folder and nothing else.

**A and B are the two that are genuinely close** — same +3 month shift, same eleven re-declarations, differing only in whether the shifted date is exposed to the user. If anything here is ever collapsed, it is that pair, not B into C. B extends a *fact table* in AQL; C extends the *date dimension* in `@sql`. They are opposite architectures, and B existing is what makes C's cost argument visible rather than asserted.
...
Keep the three solution folders independent. A change that makes two of them share an object is a regression.
