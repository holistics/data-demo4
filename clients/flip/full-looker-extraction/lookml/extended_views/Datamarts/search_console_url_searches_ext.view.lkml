include: "/base_views/Datamarts/search_console_url_searches_base.view"

view: search_console_url_searches_ext {
  extends: [search_console_url_searches_base]
...
# ----- ORIGINAL DIMENSIONS ---------------------------------------------------------------------------------------------------------------------------------------
  ...
  measure: impressions_sum {
    ...
    value_format: "#,##0"
    drill_fields: [query, search_type, search_brand_type, url, impressions_sum, clicks_sum, ctr_percent, avg_position]
  }
  ...
  measure: avg_position {
    type: number
    sql: SUM(${sum_position})/SUM(${impressions}) + 1 ;;
    label: "Avg Position"
