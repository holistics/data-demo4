...
# ----- ORIGINAL DIMENSIONS ---------------------------------------------------------------------------------------------------------------------------------------
  ...
  dimension: hs_associated_contact_name {
    type: string
    link: {
      label: "HubSpot URL"
      url: "https://app.hubspot.com/contacts/7401529/contact/{{ hs_associated_contact_id }}/view/1?engagement={{ hs_meeting_held_id }}"
    }
  ...
  dimension: hs_associated_deal_stage {
    type: string
  }
  dimension: hs_associated_deal_type {
    type: string
  }
  dimension: hs_meeting_held_id {
    type: string
  }
  dimension: hs_meeting_location {
    type: string
  }
...
# ----- MEASURES --------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  measure: meetings_held_count {
    ...
    label: "# Meetings Held"
    drill_fields: [sl_started_at_berlin_timestamp_time, sl_meeting_owner, sl_meeting_duration_min, sl_created_via, sl_meeting_title, hs_associated_company_name, hs_associated_company_segment, hs_associated_contact_name, hs_associated_deal_name]
  }
