...
- **Returns nothing** → stop and invoke `scaffold-semantic-layer` to create the models,
...
have been two, record it and send it back to `scaffold-semantic-layer` — do not fix it inline, or
...
`../new-analytics-onboarding/references/data-quality-register.md` for the naming, front matter, lifecycle, and index conventions;
...
should be split. Each one is a `scaffold-semantic-layer` task, not something you fix here.}
