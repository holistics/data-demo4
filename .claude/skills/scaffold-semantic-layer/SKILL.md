...
Create the folders from the **Default AMQL project layout** in the `new-analytics-onboarding`
orchestrator, which owns that convention. Create only the folders you will actually fill; an empty
...
- **`docs/data-quality/`** — open a `dq-NNN` entry for every structural defect you found but did not
  ...
  not confirm. Use `../new-analytics-onboarding/references/data-quality-register.md` for the naming, front matter, and index
